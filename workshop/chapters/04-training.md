...
>
> **소요 시간**: 약 75분 (훈련 대기)
> **목표**: PPO 알고리즘으로 ANYmal-C의 rough terrain 보행 정책을 학습합니다.
...
| `/workspace/isaaclab/logs` | **체크포인트 저장 위치** |

> 🚨 **DANGER**
>
> **소스 마운트 금지**: `-v "/opt/isaaclab/source:/workspace/isaaclab/source:rw"`를 하지 마세요. editable install(`pip install -e`)의 `.pth` 참조가 깨져서 `ModuleNotFoundError`가 발생합니다.
...
로봇의 행동을 평가하는 **보상 함수**가 학습의 핵심입니다:
...
> **핵심**: 보상 함수 설계가 로봇의 최종 행동을 결정합니다. 속도 추적만 보상하면 에너지를 과다 사용하고, 에너지 패널티만 주면 움직이지 않습니다. 이 균형을 맞추는 것이 **reward engineering**입니다.

---

## 4.5 커리큘럼 학습

Isaac Lab은 **커리큘럼 학습**을 자동으로 수행합니다:
...
> ✅ **SUCCESS**
>
> **성공 기준**: 최종 Mean Reward > +10, Episode Length > 500이면 로봇이 안정적으로 보행하는 것입니다.
